...
if (typeof (window.PMP) == "undefined") {
    ...
    PMP.RavenInit = function (entryCode, customerId, needDownloadRaven, resourceListen, ajaxListen) {
        ...
        if (needDownloadRaven) {
            PMP.JSLoader.load("https://res-static.hc-cdn.cn/aem/content/dam/cloudbu-site/archive/commons/web_resoure/framework/portal/pmp/raven.min.js?v=2.0", function () {
                if (entryCode != "" && (typeof Raven != "undefined")) {
                    //初始化上报url
                    Raven && Raven.config(entryCode, { autoBreadcrumbs: { console: false }, maxBreadcrumbs: 50, allowDuplicates: true }).install();
                    //设置userid
            ...
            if (entryCode != "" && (typeof Raven != "undefined")) {
                //初始化上报url
                Raven && Raven.config(entryCode, { autoBreadcrumbs: { console: false }, maxBreadcrumbs: 50, debug: true, allowDuplicates: true }).install();
                //设置userid
                PMP.RavenSetCustomerID(customerId);
                //添加监听
                if (typeof resourceListen == 'undefined') {
            ...
            window.addEventListener('error', function (error) {
                ...
                if (!error.message && !(!!window.ActiveXObject || "ActiveXObject" in window)) {
                    ...
                    if (window.location.host) {
                        ...
                        if (subDomain == "www.huaweicloud.com" || subDomain == "activity.huaweicloud.com" || subDomain == "support.huaweicloud.com" || subDomain == "bbs.huaweicloud.com") {
                            //官网资源异常，不在此处上报
                            return true;
                        }
                    }
